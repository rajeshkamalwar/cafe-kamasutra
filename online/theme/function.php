...
{
    $return='';
    $result=$mysqli->query("Select `cat_name_en` From `categories` WHERE `cat_id`='$cat_id'");
...
{
    $return='';
    $result=$mysqli->query("Select `cat_id` From `categories` WHERE `cat_name_en`='$cat_name'");
    $row = $result->fetch_assoc();
    $return = $row['cat_id'];
    return $return;
}
...
{
    $return='';
    $result=$mysqli->query("Select `cat_id`,`cat_name_en` From `categories` Where `cat_status`='Active'");
