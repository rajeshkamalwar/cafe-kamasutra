...
include 'conform_user.php'; 
  ...
  if ($print_dish_check->num_rows > 0) {
 	 $row1 = $print_dish_check->fetch_assoc();
	  $varorder = $row1['varialbe_order'];
	  
	  $array=array_map('intval', explode(',', $varorder));
	   $array22=array_map('intval', explode(',', $varorder));
    $array = implode("','",$array); 
   $print_dish = "SELECT  *  FROM `variable`  WHERE    `variable_id` IN('".$array."') ORDER BY FIELD(`variable_id`,'" . $array . "')	 ";  
	  $result_cat_query1 = $mysqli->query($print_dish);
	  
	  $all_varar = [];
	   $all_var_qury = "SELECT * from  `variable`";     
       $allvarget = $mysqli->query($all_var_qury); 
 		 while ($row1 = $allvarget->fetch_assoc()) {
			 $all_varar[] = $row1['variable_id'];
		 }
	  
	 $result=array_diff($all_varar,$array22);
		 
		  
	 foreach($result as $result_var){
		  $all_var_qury_2 = "SELECT * from  `variable` WHERE `variable_id` = $result_var";     
       $allvarget_2 = $mysqli->query($all_var_qury_2); 
		 $row1_result = $allvarget_2->fetch_assoc();
		$list_dish .= '<li id="' . $row1_result['variable_id'] . '" draggable="true">' . $row1_result['variable_name_en'] . '</li>'; 
	 }
	  
	    while ($row1 = $result_cat_query1->fetch_assoc()) {
            $list_dish .= '<li id="' . $row1['variable_id'] . '" draggable="true">' . $row1['variable_name_en'] . '</li>';
        }
  }

else{

  $final_query = "SELECT * from  `variable`";     
            ...
            <script>
                ...
                $(document).ready(function () {
                    $('#save').click(function () {
                        ...
                        url = b_url + 'varialbe-prder-action.php';
