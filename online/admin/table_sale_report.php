                                     ...
                                     if(isset($_POST['submitdate'])){
										...
										$d = date('Y-m-d 23:00:00', strtotime($date));
										 $d2 = date('Y-m-d 00:00:00', strtotime($date2));
											 $additional_query = "AND `order_date` >= '$d2' AND `order_date`<= '$d' ";
										}
       			

								/* New */
								
								 $list_gift_query = "Select * From `admin_orders_new` where ad_status='1' ".$additional_query."  ORDER BY `id` desc";
									$query_result66 = $mysqli->query($list_gift_query); 
									$count = $count+ $query_result66->num_rows;
									while($row11 = $query_result66->fetch_assoc()){
								  $totalamt = $totalamt + $row11['price_after_dis'];
									}
								
								 $list_gift_query11 = "Select * From `admin_orders_new` where ad_status='1' and paid_with = 'card' ".$additional_query." ORDER BY `id` desc";
									$query_result6611 = $mysqli->query($list_gift_query11); 
									$count11 =  $count11 + $query_result6611->num_rows;
									while($row111 = $query_result6611->fetch_assoc()){
								  $totalamt1 = $totalamt1 + $row111['price_after_dis'];
									}
									 $list_gift_query12 = "Select * From `admin_orders_new` where ad_status='1' and paid_with = 'cash' ".$additional_query." ORDER BY `id` desc";
									$query_result6612 = $mysqli->query($list_gift_query12); 
									$count12 =$count12 +  $query_result6612->num_rows;
									while($row112 = $query_result6612->fetch_assoc()){
								  $totalamt2 = $totalamt2 + $row112['price_after_dis'];
									}

								 $list_gift_query13 = "Select * From `admin_orders_new` where ad_status='1' and paid_with = 'pin' ".$additional_query." ORDER BY `id` desc";
									$query_result6613 = $mysqli->query($list_gift_query13); 
									$count13 =$count13 + $query_result6613->num_rows;
									while($row113 = $query_result6613->fetch_assoc()){
								  $totalamt3 = $totalamt3 + $row113['price_after_dis'];
									}
                                ...
                                <div class="box-body table-responsive no-padding">
                                    ...
                                    <div class="col-sm-12" style="margin-top: 35px;" id="dvContents">
                                     ...
                                     <div class="col-sm-2" style="border: solid #4f4fa4; text-align: center;">
										 <h4>Total Order</h4>
										 <p><?php echo $count;?></p>
										<h5>Total Amount:</h5> <p><?php echo currency . ' '; ?> <?php echo str_replace('.',',',number_format((float)$totalamt,2)); ?>	</p>
										 
									 </div>
										<div class="col-sm-2" style="border: solid #4f4fa4; text-align: center;">
									    <h4>Order ( card Payment )</h4>
										 <p><?php echo $count11;?></p>
										 <h5>Total Amount:</h5> <p><?php echo currency . ' '; ?> <?php echo str_replace('.',',',number_format((float)$totalamt1,2)); ?>	</p>
									 </div>
