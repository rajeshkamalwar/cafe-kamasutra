...
$totlacatid = array();
 ...
 $list_cat_query = "Select `supcat_id`,`supcat_name_en` From `tsupercategories`";
$menu_order_query = "Select `sup_cat_order` from `tsupercategory_order` where `supcatorder_id`='1'";
...
//no cat found
    ...
    while ($row = $result_cat_query->fetch_assoc()) {
        array_push($totlacatid, $row['supcat_id']);
    }
    $cat_sortorderlist = $result_menu_order_query->fetch_assoc();
    if (strlen($cat_sortorderlist['sup_cat_order']) > 0) {
        $cat_sortorderlist1 = explode(",", $cat_sortorderlist['sup_cat_order']);
    ...
    $final_query = '';
    if (!empty($finalstr)) {
        $final_query = "SELECT `supcat_id`,`supcat_name_en` FROM `tsupercategories` WHERE `supcat_id` IN (" . $finalstr . ") ORDER BY FIELD(`supcat_id`," . $finalstr . ")";
    }
    
    $result_cat_query1 = $mysqli->query($final_query);
    if ($result_cat_query1->num_rows > 0) {
        while ($row1 = $result_cat_query1->fetch_assoc()) {
            $list_dish .= '<li id="' . $row1['supcat_id'] . '" draggable="true">' . $row1['supcat_name_en'] . '</li>';
        }
    }
}
?>
            ...
            <script>
                ...
                $(document).ready(function () {
                    $('#save').click(function () {
                        ...
                        url = b_url + 'torder_supcat_action.php';
