...
if (isset($_POST['action'])) {
    ...
    if ($action == "load") {
        $list_postcode_query        = "Select * From `lostcustomercoupon` where id!= '1' ORDER BY id DESC";
...
if(!empty($rest_info)){ $newrssinfo = $rest_info; }	else { $newrssinfo=''; }
		...
		$query11 = $mysqli->query("select DISTINCT usr_emailid from tbl_user where ddate >= DATE_SUB(CURDATE(), INTERVAL $days DAY)");
		$count = $query11->num_rows;
		
        $edit_query_updte_postcode = "INSERT INTO `lostcustomercoupon` SET `customerdays`='" . $days . "', `discount`='" . $discount . "',`per_amount`='" . $per_amount . "',`fix_amount`='" . $fix_amount . "',`validdays`='" . $validdays . "',`couponcode`='".$coupon."',`noofusers`='".$count."',`restra_holi_en`='".$restra_holi_en."',`delivery_type`='".$delivery_type."'";
