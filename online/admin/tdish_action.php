...
function getallupdatedCatid($dish_cat_list_edit, $mysqli, $dish_id) {
    ...
    for ($counti = 0; $counti < $no_of_cat; $counti++) {
       ...
       // echo "<br/>" . $chk_in_odrdis_tab;
        ...
        $query1="SELECT `dish_id` FROM `tdish` WHERE CONCAT(',', `categry_id`, ',') like '%,".$dish_cat_list_edit[$counti].",%' ORDER By `dish_id` ASC";
...
if (isset($_POST['dish_action'])) {
     ...
     if ($dish_action == 'add_load') {
        ...
        $avilable_variable = "Select * from `tvariable` where variable_status='Active'";
    ...
    if ($dish_action == "load") {
        $list_dish_query = "Select * From `tdish`";
...
if ($dish_action == "viewimage") {
	 $result = $mysqli->query("SELECT * FROM `tdish` WHERE `dish_id`='" . $_POST['dish_id'] . "'");
...
$icon = $_POST['icon_edit'];
        $edit_dish_query = "UPDATE `tdish` SET `dish_name_en`='" . $dish_name_edit_en . "',`dish_name_nl`='" . $dish_name_edit_nl . "',`dish_desc_en`='" . $dish_description_edit_en . "',`dish_desc_nl`='" . $dish_description_edit_nl . "',`dish_price`='" . $dish_price_edit . "',`categry_id`='" . $dish_cat_list_edit . "',`dish_type`='" . $dish_type . "',`dish_attrib`='" . $opt_var_list_edit . "',`dish_tax_rate`='" . $tax_percent_edit . "',`icon`='".$icon."' WHERE `dish_id`='" . $dish_id . "'";

        // die($edit_dish_query);
        $dupesql = "SELECT * FROM `tdish` where `dish_name_en` = '$dish_name_edit_en'";
        $duperaw = $mysqli->query($dupesql);       //echo "<pre>";print_r($duperaw);echo "</pre>";die();
        $duperaw_row = $duperaw->fetch_assoc();
        $notification_message = '';
    ...
    if ($dish_action == 'get_status') {
        ...
        $dish_id = $_POST['dish_id'];
        $query = "select `dish_status` from `tdish` where dish_id=" . $dish_id;
...
<input id="mso" type="hidden" value="' . $dish_id . '"/>
    ...
    if ($dish_action == 'change_status') {
        ...
        $change_status_query = "UPDATE `tdish` SET `dish_status`='" . $status . "' WHERE `dish_id`='" . $dish_id . "'";
    ...
    if ($dish_action == 'delete') {
        $dish_id = $_POST['dish_id'];
        $notification_message = '';
        $query = "DELETE  FROM `tdish` WHERE `dish_id`='" . $dish_id . "'";
...
if ($dish_action == 'deleteimage') {
        $dish_id = $_POST['dish_id'];
        $notification_message = '';
        $query = "update `tdish` SET `product_image`='',`thumbnail`='' WHERE `dish_id`='" . $dish_id . "'";
...
<script>
	$(document).on('click', '#change_status_btn', function () {
		...
		//alert(dish_id);
                    url = b_url + 'tdish_action.php';  //console.log(url);  console.log();
                    $.ajax({
                        type: "POST",
                        url: url,
                        data: {
