    ...
    if ($gift_action == "load") {
      ...
      ///  $additional_query="";
       /// if($showallodr!="yes"){$additional_query=" AND ot_trx_status='Success' ";}
        ...
        $list_gift_query = "Select * From `tbl_orders`  where `ot_OrderDate` like '" . date('Y') . "%'".$additional_query." ORDER BY `ot_UserId` desc"; //echo $list_gift_query;die();
        ...
        if ($result_list_gift_query->num_rows == 0) {
            ...
            while ($row = $result_list_gift_query->fetch_assoc()) {
				...
				if($gpstype=='Mail'){
					...
					$gstaction = '<a href="mailsend.php?id='.$row['ot_UserId'].'&otid='.$row['ot_id'].'&m=3">Send Mail</a>';
						}
				}else{
					if($row['gpsstatus']=='1'){
						$gstaction = 'Sent';
                ...
                $user_details='<table class="table table-bordered table-striped"><tr>
                 ...
                 while ($row_usr = $result_usr_detal_query->fetch_assoc()) {
					 				if($row['ot_paymentoption']=='COD'){$paymentMethod='CASH';}elseif($row['ot_paymentoption']=='creditcard'){$paymentMethod='Master Card';}else{$paymentMethod=$row['ot_paymentoption'];}
					if($row['cutlery']=='yes'){
				...
				if($row['ot_status']!='Complete'){
						 $orderstatus = $row['ot_status']. "<br/>" .'<a href="updateorderstatus.php?id='.$row['ot_UserId'].'&otid='.$row['ot_id'].'&m=3">click for Complete</a>';
					 } else { 
     ...
     if ($gift_action == "edit_load_record") {
        ...
        $paymentactive1=$paymentactive2=$paymentactive3=$paymentactive5=$paymentactive3='';
...
if ($gift_action == 'delete') {
    ...
    if ($gift_action == "edit") {
     ...
     if ($gift_action == "print") {
        ...
        $edit_gift_query_result = $mysqli->query($edit_query);
        $logo_url="http://littleindiarestaurant.nl/site/wp-content/themes/onlineorder/img/logo.jpg";
        $ot_id = $_POST['ot_id'];
