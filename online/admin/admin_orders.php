...
<?php
        ...
        $list_gift = '<tbody><tr>
				...
				$list_gift_query = "Select * From `admin_orders_new`   ORDER BY `id` desc";  
          ...
          while ($row = $result_list_gift_query->fetch_assoc()) {
			...
			$list_gift .= '<tr  class="listrecord'.$row['id'].'">          <td>' . $cno . '</td>               
			   ...
			   <td>'. date_format(new DateTime($row["date_time"]), "H:i").'</td>
			  
			   <td><span>' . $row['paid_with'] . '</span><div class="updatebox"  style="display:none;"><select class="orderopt"><option>-</option><option vlaue="Pin">PIN</option><option value="Cash">Cash</option><option value="Card">Card</option></select><button type="button" class="saveorderpayment"  data-orderrow="'.$row['id'].'"  data-newid="1">Submit</button><input type="hidden" class="payopt"></div></td>
			...
			echo $list_gift;				
		     ...
		     $list_gift_query2 = "Select * From `admin_orders`   ORDER BY `id` desc";
			   $result_list_gift_query2 = $mysqli->query($list_gift_query2);
            while ($row = $result_list_gift_query2->fetch_assoc()) {
			$list_gift2 .= '<tr  class="listrecord'.$row['id'].'">          <td>' . $cno . '</td>               
			   <td>' . $row['table_no'] . '</td>
			  
			   <td>'. date_format(new DateTime($row["order_date"]), "M d, Y").'</td>
			   <td>'  . date_format(new DateTime($row["order_time"]), "H:i") . '</td>
			   <td><span>' . $row['paid_with'] . '</span><div class="updatebox"  style="display:none;"><select class="orderopt"><option>-</option><option vlaue="Pin">PIN</option><option value="Cash">Cash</option><option value="Card">Card</option></select><button type="button" class="saveorderpayment"  data-orderrow="'.$row['id'].'">Submit</button><input type="hidden" class="payopt"></div></td>
			   <td>' . $row['subtotal'] . '</td>
		...
		//<a href="printoption.php?dataid='.$row['id'].'" class="btn btn-social-icon btn-warning"><i class="fa fa-print"></i></a> 									?>
                                        </table>
                                    </div>
                                </div>
                                <!-- /.box-body -->
                                    ...
                                    <div class="modal-dialog">
                                        <div class="modal-content"> 
                                            ...
                                            <div class="modal-footer">
												  <input type="hidden" value="0" id="dele_new">
