...
<button type="button" id="refersh" class="btn btn-primary "><i  class="fa fa-refresh"></i> Refresh</button>
            ...
            <script type="text/javascript">
                ...
                $(document).on('click', '#set_time_update', function () {
                    ...
                    var mon_close = '';var sun_close = '';var sat_close = '';var fri_close = '';var thu_close = '';var wed_close = '';var tue_close = '';
                    if ($("#mon_close").prop('checked') == true) {mon_close = 'close';} else {mon_close = 'open';}
                    if ($("#tue_close").prop('checked') == true) {tue_close = 'close';} else {tue_close = 'open';}
                    if ($("#wed_close").prop('checked') == true) {wed_close = 'close';} else {wed_close = 'open';}
                    if ($("#thu_close").prop('checked') == true) {thu_close = 'close';} else {thu_close = 'open';}
                    if ($("#fri_close").prop('checked') == true) {fri_close = 'close';} else {fri_close = 'open';}
                    if ($("#sat_close").prop('checked') == true) {sat_close = 'close';} else {sat_close = 'open';}
                    if ($("#sun_close").prop('checked') == true) {sun_close = 'close';} else {sun_close = 'open';}
                    ...
                    $.ajax({
                        ...
                        data: {
                            ...
                            mon_close: mon_close, tue_close: tue_close, wed_close: wed_close, thu_close: thu_close,
                            fri_close: fri_close, sat_close: sat_close, sun_close: sun_close,
                        },
                        dataType: "html",
                        success: function (data)
                        {
