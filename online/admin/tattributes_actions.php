...
if (isset($_POST['attrib_action'])) {
    ...
    if ($attrib_action == "view") {
        ...
        $result = $mysqli->query("SELECT * FROM `tattribute` WHERE `attrib_id`='" . $_POST['attrib_id'] . "'");
...
<div class="form-group">
    ...
    if ($attrib_action == "load") {
        $list_attrib_query = "Select * From `tattribute`";
    ...
    if ($attrib_action == "add") {
        ...
        $add_attrrib_query = "INSERT INTO `tattribute`(`attrib_name_en`,`attrib_name_nl`,`attrib_price`,`attrib_desc_en`,`attrib_desc_nl`) VALUES ('" . $attributes_name_new_en . "','" . $attributes_name_new_nl . "','" . $attributes_price_new . "','" . $attributes_description_add_en . "','" . $attributes_description_add_nl . "')";      //    echo $add_attrrib_query;die();
        $dupesql = "SELECT * FROM `tattribute` where `attrib_name_en` = '$attributes_name_new_en'";
...
<div class="form-group">
    ...
    if ($attrib_action == "edit") {
        ...
        $edit_attrrib_query = "UPDATE `tattribute` SET `attrib_name_en`='" . $attrib_name_en . "',`attrib_name_nl`='" . $attrib_name_nl . "', `attrib_price`='" . addZeroes($attributes_price_edit) . "',`attrib_desc_en`='" . $variable_description_edit_en . "',`attrib_desc_nl`='" . $variable_description_edit_nl . "'  WHERE `attrib_id`='" . $attrib_id . "'";

        $dupesql = "SELECT * FROM `tattribute` where `attrib_name_en` = '$attrib_name_en'";
        $duperaw = $mysqli->query($dupesql);        //echo "<pre>";print_r($duperaw);echo "</pre>";die();
        $duperaw_row = $duperaw->fetch_assoc();
    ...
    if ($attrib_action == 'get_status') {
        $attrib_id = $_POST['attrib_id'];
        $query = "select `attrib_status` from `tattribute` where attrib_id=" . $attrib_id;
...
<input id="mso" type="hidden" value="' . $attrib_id . '"/>
    ...
    if ($attrib_action == 'change_status') {
        ...
        $change_status_query = "UPDATE `tattribute` SET `attrib_status`='" . $status . "' WHERE `attrib_id`='" . $attrib_id . "'";
    ...
    if ($attrib_action == 'delete') {
        $attrib_id = $_POST['attrib_id'];
        $notification_message = '';
        $query = "DELETE  FROM `tattribute` WHERE `attrib_id`='" . $attrib_id . "'";
