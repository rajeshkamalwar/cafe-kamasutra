...
if (isset($_POST['cat_action'])) {
    ...
    if ($cat_action == "add") {
        ...
        $add_attrrib_query = "INSERT INTO `tsupercategories`(`supcat_name_en`,`supcat_name_nl`, `supcat_desc_en`, `supcat_desc_nl`) VALUES ('" . $supcat_name_new_ln1 . "','" . $supcat_name_new_ln2 . "','" . $supcat_description_add_ln1 . "','" . $supcat_description_add_ln2 . "')";
		
        //echo $add_attrrib_query;die();
        $dupesql = "SELECT * FROM `tsupercategories` where `supcat_name_en` = '$supcat_name_new_ln1'";
    ...
    if ($cat_action == "load") {
        $list_attrib_query = "Select * From `tsupercategories`";
    ...
    if ($cat_action == "view") {
        $cat_view_query = "Select * From `tsupercategories` where `supcat_id`='" . $_POST['supcat_id'] . "'";
    ...
    if ($cat_action == "edit") {
        ...
        $edit_cat_query = "UPDATE `tsupercategories` SET `supcat_name_en`='" . $supcat_name_edit_ln1 . "',`supcat_name_nl`='" . $supcat_name_edit_ln2 . "', `supcat_desc_en`='" . $supcat_description_edit_ln1 . "',`supcat_desc_nl`='" . $supcat_description_edit_ln2 . "' WHERE `supcat_id`='" . $supcat_id . "'";

        // echo $edit_cat_query;die();

        $dupesql = "SELECT * FROM `tsupercategories` where `supcat_name_en` = '$supcat_name_edit_ln1'";
    ...
    if ($cat_action == 'get_status_cat') {
        $supcat_id = $_POST['supcat_id'];
        $query = "select `supcat_status` from `tsupercategories` where supcat_id=" . $supcat_id;
...
<input id="mso" type="hidden" value="' . $cat_id . '"/>
    ...
    if ($cat_action == 'change_status') {
        ...
        $change_status_query = "UPDATE `tsupercategories` SET `supcat_status`='" . $status . "' WHERE `supcat_id`='" . $cat_id . "'";
    ...
    if ($cat_action == 'delete') {
        $supcat_id = $_POST['supcat_id'];
        $notification_message = '';
        $query = "DELETE  FROM `tsupercategories` WHERE `supcat_id`='" . $supcat_id . "'";
