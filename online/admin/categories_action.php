...
if (isset($_POST['cat_action'])) {
    ...
    if ($cat_action == "add") {
        ...
        $add_attrrib_query = "INSERT INTO `categories`(`sub_cat_id`,`cat_name_en`,`cat_name_nl`, `cat_desc_en`, `cat_desc_nl`) VALUES ('".$supcat_name_new_ln2."','" . $cat_name_new_ln1 . "','" . $cat_name_new_ln2 . "','" . $cat_description_add_ln1 . "','" . $cat_description_add_ln2 . "')";
        //echo $add_attrrib_query;die();
        $dupesql = "SELECT * FROM `categories` where `cat_name_en` = '$cat_name_new_ln1'";
    ...
    if ($cat_action == "load") {
        $list_attrib_query = "Select * From `categories`";
    ...
    if ($cat_action == "view") {
        $cat_view_query = "Select * From `categories` where `cat_id`='" . $_POST['cat_id'] . "'";
        $result_cat_view_query = $mysqli->query($cat_view_query);
        $row = $result_cat_view_query->fetch_assoc();
		$dupesql22 = "SELECT * FROM `supercategories` where supcat_id = '". $row['sub_cat_id'] ."' ";
    ...
    if ($cat_action == "edit_load_record") {
        ...
        $result_cat_view_query = $mysqli->query($cat_view_query);
        $row = $result_cat_view_query->fetch_assoc();
			$dupesql2 = "SELECT * FROM `supercategories`";
    ...
    if ($cat_action == "edit") {
        ...
        $edit_cat_query = "UPDATE `categories` SET `cat_name_en`='" . $cat_name_edit_ln1 . "',`cat_name_nl`='" . $cat_name_edit_ln2 . "', `cat_desc_en`='" . $cat_description_edit_ln1 . "',`cat_desc_nl`='" . $cat_description_edit_ln2 . "',`sub_cat_id`='".$supcat_name_edit_ln2."'  WHERE `cat_id`='" . $cat_id . "'";

        // echo $edit_cat_query;die();

        $dupesql = "SELECT * FROM `categories` where `cat_name_en` = '$cat_name_edit_ln1'";
    ...
    if ($cat_action == 'get_status_cat') {
        $cat_id = $_POST['cat_id'];
        $query = "select `cat_status` from `categories` where cat_id=" . $cat_id;
...
<input id="mso" type="hidden" value="' . $cat_id . '"/>
    ...
    if ($cat_action == 'change_status') {
        ...
        $change_status_query = "UPDATE `categories` SET `cat_status`='" . $status . "' WHERE `cat_id`='" . $cat_id . "'";
        ...
        if (strlen($cat_sortorderlist['cat_sort_order']) > 0) {
            ...
            for ($counti = 0; $counti < $sizeofarray; $counti++) {
                if ($cat_sortorderlist1[$counti] == $cat_id) {
                    ...
                    $newdortorder4cat_query = "UPDATE `menu_order` SET `cat_sort_order`='".$newdortorder4cat."' WHERE `cat_sortorder_id`='1'";$mysqli->query($newdortorder4cat_query);
