var yay_smtp_sendgrid_prefix = "yay_smtp_sendgrid";
var yay_smtp_sendgrid_class_obj = ".yay_smtp_sendgrid";
(function($) {
  $(document).ready(function() {
    // Catch Pagination per page event
    $(
      yay_smtp_sendgrid_class_obj + ".yay-smtp-wrap.mail-logs .pag-per-page-sel"
    ).change(function() {
      let param = searchYaySmtpSendgridConditionBasicCurrent();
      param.sortField = yay_smtp_sendgrid_sort_field;
      param.sortVal = yay_smtp_sendgrid_sort_val;
      YaySmtpSendgridEmailLogsList(param);
    });

    // Catch current page event
    $(
      yay_smtp_sendgrid_class_obj + ".yay-smtp-wrap.mail-logs .pag-page-current"
    ).change(function() {
      let param = searchYaySmtpSendgridConditionBasicCurrent();
      param.sortField = yay_smtp_sendgrid_sort_field;
      param.sortVal = yay_smtp_sendgrid_sort_val;
    ...
    $(
      ...
      let status;
      if (
        $(yay_smtp_sendgrid_class_obj + " #yaysmtp_logs_status_not_send").is(
          ":checked"
        )
      ) {
        if (
          $(yay_smtp_sendgrid_class_obj + " #yaysmtp_logs_status_sent").is(
            ":checked"
          )
        ) {
          status = "all";
    ...
    // Catch search even
    var yaysmtpsendgridTimeout = null;
    $(
      yay_smtp_sendgrid_class_obj +
        ".yay-smtp-wrap.mail-logs .yay-button-wrap .search .search-imput"
    ).keyup(function(e) {
      clearTimeout(yaysmtpsendgridTimeout);
      yaysmtpsendgridTimeout = setTimeout(function() {
        let param = searchYaySmtpSendgridConditionBasicCurrent();
        param.sortField = yay_smtp_sendgrid_sort_field;
        param.sortVal = yay_smtp_sendgrid_sort_val;
        YaySmtpSendgridEmailLogsList(param);
    ...
    $(yay_smtp_sendgrid_class_obj + " #yaysmtp_logs_subject_control").click(
      function() {
        let show_subj_cl;
        if ($(this).is(":checked")) {
          $(
        ...
        //Update DB
        $.ajax({
          url: yay_smtp_sendgrid_wp_data.YAY_ADMIN_AJAX,
          type: "POST",
          // async: false,
          data: {
            action: yay_smtp_sendgrid_prefix + "_set_email_logs_setting",
            nonce: yay_smtp_sendgrid_wp_data.ajaxNonce,
    ...
    $(yay_smtp_sendgrid_class_obj + " #yaysmtp_logs_to_control").click(
      function() {
        let show_to_cl;
        if ($(this).is(":checked")) {
          $(
    ...
    $(yay_smtp_sendgrid_class_obj + " #yaysmtp_logs_status_control").click(
      function() {
        let show_status_cl;
        if ($(this).is(":checked")) {
          $(
    ...
    $(yay_smtp_sendgrid_class_obj + " #yaysmtp_logs_datetime_control").click(
      function() {
    ...
    $(yay_smtp_sendgrid_class_obj + " #yaysmtp_logs_action_control").click(
      function() {
        let show_action_cl;
        if ($(this).is(":checked")) {
          $(
    ...
    // Catch sent control
    $(yay_smtp_sendgrid_class_obj + " #yaysmtp_logs_status_sent").click(
      function() {
        let status;
        if ($(this).is(":checked")) {
          if (
            $(
              yay_smtp_sendgrid_class_obj + " #yaysmtp_logs_status_not_send"
            ).is(":checked")
          ) {
            status = "all";
          } else {
    ...
    $(yay_smtp_sendgrid_class_obj + " #yaysmtp_logs_status_not_send").click(
      function() {
        let status;
        if ($(this).is(":checked")) {
          if (
    ...
    // Sorting - start
    var yay_smtp_sendgrid_sort_field = "";
    var yay_smtp_sendgrid_sort_val = "";
    $(
      yay_smtp_sendgrid_class_obj + " .yay-smtp-content thead th.is-sortable"
    ).click(function() {
      ...
      // Update global val for yay_smtp_sendgrid_sort_field, yay_smtp_sendgrid_sort_val
      yay_smtp_sendgrid_sort_field = sortField;
      yay_smtp_sendgrid_sort_val = sortValReal;
    ...
    // Delete item action
    $(yay_smtp_sendgrid_class_obj + ".yay-smtp-wrap.mail-logs").on(
      "click",
      ".yaysmtp-delete-btn",
      function() {
        if (confirm("Are you sure to delete this item?")) {
          ...
          $.ajax({
            ...
            type: "POST",
            data: {
              action: yay_smtp_sendgrid_prefix + "_delete_email_logs",
              nonce: yay_smtp_sendgrid_wp_data.ajaxNonce,
              params: {
                ids: idMailLog
              }
            ...
            beforeSend: function() {
              YaySmtpSendgridSpinner("yay-smtp-wrap", true);
            },
            success: function(result) {
              if (result.success) {
                let param = searchYaySmtpSendgridConditionBasicCurrent();
              ...
              YaySmtpSendgridNotification(result.data.mess, "yay-smtp-wrap");
              YaySmtpSendgridSpinner("yay-smtp-wrap", false);
            }
          });
        }
      }
    ...
    $(yay_smtp_sendgrid_class_obj + ".yay-smtp-wrap.mail-logs").on(
      ...
      function() {
        let idMailLogs = [];
        let mailLogCheckeds = $(
          yay_smtp_sendgrid_class_obj + ".yay-smtp-wrap.mail-logs"
      ...
      function() {
        if (confirm("Are you sure to delete all mail logs?")) {
          $.ajax({
            ...
            type: "POST",
            data: {
              action: yay_smtp_sendgrid_prefix + "_delete_all_email_logs",
              nonce: yay_smtp_sendgrid_wp_data.ajaxNonce,
              params: {}
            },
            beforeSend: function() {
    ...
    $(
      yay_smtp_sendgrid_class_obj + ".yay-smtp-wrap.mail-logs #input-check-all"
    ).click(function() {
      if ($(this).is(":checked")) {
        $(
          yay_smtp_sendgrid_class_obj +
    ...
    if (
      $(yay_smtp_sendgrid_class_obj + ".yay-smtp-wrap.mail-logs").find(
      ...
      function(e) {
        ...
        if ($(this).hasClass("is-active")) {
          ...
          $.ajax({
            ...
            type: "POST",
            data: {
              action: yay_smtp_sendgrid_prefix + "_detail_email_logs",
              nonce: yay_smtp_sendgrid_wp_data.ajaxNonce,
              params: {
                id: idEmailLog
              }
            ...
            beforeSend: function() {
              YaySmtpSendgridSpinner("yay-smtp-mail-detail-drawer", true);
            },
            success: function(result) {
              if (result.success) {
                ...
                if (typeof data.body_content !== "undefined") {
                  ...
                  let bodyContent = yaysmtp_sendgrid_wrap_url(bodyContentRep)
              ...
              YaySmtpSendgridSpinner("yay-smtp-mail-detail-drawer", false);
            }
          });
        }
      }
    ...
    $(
      yay_smtp_sendgrid_class_obj + " .yay-smtp-mail-detail-drawer .closebtn"
    ).click(function() {
      $(yay_smtp_sendgrid_class_obj + ".yay-smtp-wrap.mail-logs")
        .find(".yay-smtp-mail-detail-drawer")
        .css("width", "0");
    ...
    $(
      yay_smtp_sendgrid_class_obj + " .yay-smtp-email-log-settings-save-action"
    ).click(function() {
      let logSettings = {};
      if (
        $(yay_smtp_sendgrid_class_obj + " #yay_smtp_mail_log_setting_save").is(
          ":checked"
        )
      ) {
    ...
    // Panel click
    $(yay_smtp_sendgrid_class_obj + " .send-test-mail-panel").click(function() {
      if ($(this).hasClass("is-active")) {
        // Close drawer
        $(yay_smtp_sendgrid_class_obj + ".yay-smtp-wrap")
    ...
    // Close panel
    $(
      yay_smtp_sendgrid_class_obj + " .yay-smtp-test-mail-drawer .closebtn"
    ).click(function() {
      $(yay_smtp_sendgrid_class_obj + ".yay-smtp-wrap")
        .find(".yay-smtp-test-mail-drawer")
        .css("width", "0");
      $(yay_smtp_sendgrid_class_obj + ".yay-smtp-wrap")
        .find(".yay-smtp-test-mail-drawer")
        .removeClass("is-open");
      $(yay_smtp_sendgrid_class_obj + " .send-test-mail-panel").removeClass(
        "is-active"
      );
    });
    ...
    $(document).mousedown(function(e) {
      ...
      let container = $(
        yay_smtp_sendgrid_class_obj + " .yay-smtp-test-mail-drawer"
      );
      let testMailPanel = $(
        yay_smtp_sendgrid_class_obj + " .send-test-mail-panel"
      );
      let svgMail = $(yay_smtp_sendgrid_class_obj + ' svg[data-icon="mail"]');
      let iconMailText = $(
        yay_smtp_sendgrid_class_obj + " .send-test-mail-panel span.text"
      );
      ...
      let containerEmailLogDetail = $(
        yay_smtp_sendgrid_class_obj + " .yay-smtp-mail-detail-drawer"
      );
      if (
      ...
      let containerEmailLogSettings = $(
        yay_smtp_sendgrid_class_obj + " .yay-smtp-mail-log-settings-drawer"
      );
    ...
    $(yay_smtp_sendgrid_class_obj)
      ...
      .on("input", function() {
        ...
        if ($(this).val().length > 0) {
          elMessage.html("").hide();
          YaySmtpSendgridValidateEmail($(this).val(), elMessage);
        } else {
    ...
    if (
      $("a.toplevel_page_yaysmtp-sendgrid").length > 0 &&
      yay_smtp_sendgrid_wp_data.succ_sent_mail_last == "no"
    ) {
      $("a.toplevel_page_yaysmtp-sendgrid .wp-menu-name").append(
    ...
    //Send Test email
    $(yay_smtp_sendgrid_class_obj + " .yay-smtp-send-mail-action").click(
      function() {
        let elMessage = $(
          yay_smtp_sendgrid_class_obj +
        ...
        let emailAddress = $(
          yay_smtp_sendgrid_class_obj + " #yay_smtp_test_mail_address"
        ).val();
        if (!emailAddress) {
          elMessage.html("Email Address is not empty!").show();
        } else {
          elMessage.html("").hide();
          if (YaySmtpSendgridValidateEmail(emailAddress, elMessage)) {
            $.ajax({
              url: yay_smtp_sendgrid_wp_data.YAY_ADMIN_AJAX,
              type: "POST",
              data: {
                action: yay_smtp_sendgrid_prefix + "_send_mail",
                nonce: yay_smtp_sendgrid_wp_data.ajaxNonce,
                emailAddress: emailAddress
              },
              beforeSend: function() {
                YaySmtpSendgridSpinner("yay-smtp-test-mail-drawer", true);
              },
              success: function(result) {
                if (!result.success) {
                  ...
                  $(yay_smtp_sendgrid_class_obj + " .yay-smtp-debug-text").html(
                    html
                  );
                  $(yay_smtp_sendgrid_class_obj + " .yay-smtp-debug").show();

                  // Show error in main page.
                  $(
                    yay_smtp_sendgrid_class_obj + " .yay-smtp-card-debug-text"
                  ).html(debugText);
                  $(
                    yay_smtp_sendgrid_class_obj +
                  ...
                  if (
                    $("a.toplevel_page_yaysmtp-sendgrid").length > 0 &&
                    $(
                      ".toplevel_page_yaysmtp-sendgrid .icon-yaysmtp-sent-mail-error"
                    ).length == 0
                  ) {
                    $("a.toplevel_page_yaysmtp-sendgrid .wp-menu-name").append(
                      '<span class="icon-yaysmtp-sent-mail-error">!</span>'
                  ...
                  YaySmtpSendgridNotification(
                    "Can not send test email",
                    "yay-smtp-wrap"
                  );
                } else {
                  elMessage.html("").hide();
                  $(yay_smtp_sendgrid_class_obj + " .yay-smtp-debug").hide();
                  $(
                    yay_smtp_sendgrid_class_obj +
                      " .yay-smtp-card.yay-smtp-card-debug"
                  ).hide();
                ...
                YaySmtpSendgridSpinner("yay-smtp-test-mail-drawer", false);
              },
              error: function(xhr, status, error) {
                console.log(xhr, status, error);
                if (yay_smtp_sendgrid_wp_data.succ_sent_mail_last == "yes") {
                  $(
                    ".toplevel_page_yaysmtp-sendgrid .icon-yaysmtp-sent-mail-error"
                  ).remove();
                }
    ...
    $(yay_smtp_sendgrid_class_obj + " .yay-smtp-save-settings-action").click(
      function() {
        let fromEmail = $(
          yay_smtp_sendgrid_class_obj + " #yay_smtp_setting_mail_from_email"
        ).val();
        let fromName = $(
          yay_smtp_sendgrid_class_obj + " #yay_smtp_setting_mail_from_name"
        ).val();
        let mailerProvider = "sendgrid"; //$("select.smtper-choose option:checked").val();
        let mailerSettings = {};
        ...
        if (!mailerProvider) {
          ...
          $.ajax({
            ...
            type: "POST",
            data: {
              action: yay_smtp_sendgrid_prefix + "_save_settings",
              nonce: yay_smtp_sendgrid_wp_data.ajaxNonce,
              settings: {
                fromEmail: fromEmail,
                fromName: fromName,
    ...
    var yaySmtpSendgridCurrentPage = getYaySmtpSendgridCookie(
      "yay_smtp_sendgrid_current_page"
    );
    if (yaySmtpSendgridCurrentPage == 1 || yaySmtpSendgridCurrentPage == "") {
      $(yay_smtp_sendgrid_class_obj + ".yay-smtp-wrap.mail-logs").hide();
      $(
        yay_smtp_sendgrid_class_obj + ".yay-smtp-wrap.send-mail-settings-wrap"
      ).show();
    } else {
      $(yay_smtp_sendgrid_class_obj + ".yay-smtp-wrap.mail-logs").show();
      $(
        yay_smtp_sendgrid_class_obj + ".yay-smtp-wrap.send-mail-settings-wrap"
      ).hide();
      loadFirstYaySMTPSendgridLogsList();
    }
    $(
      yay_smtp_sendgrid_class_obj +
      ...
      setYaySmtpSendgridCookie("yay_smtp_sendgrid_current_page", 2, 1); // Mail logs page
      loadFirstYaySMTPSendgridLogsList();
    });
    $(yay_smtp_sendgrid_class_obj + " .mail-setting-redirect, " + yay_smtp_sendgrid_class_obj + " .dashicons-arrow-left-alt").click(function(
      e
    ) {
      e.preventDefault();
      $(yay_smtp_sendgrid_class_obj + ".yay-smtp-wrap.mail-logs").hide();
      ...
      setYaySmtpSendgridCookie("yay_smtp_sendgrid_current_page", 1, 1); // Mail settings page
    });
  });
})(window.jQuery);

function YaySmtpSendgridValidateEmail(mail, elMessage) {
  if (
...
function YaySmtpSendgridSpinner(containerClass, isShow) {
...
function YaySmtpSendgridNotification(messages, containerClass) {
  let notifyHtml =
...
function YaySmtpSendgridEmailLogsList(param) {
  jQuery.ajax({
    url: yay_smtp_sendgrid_wp_data.YAY_ADMIN_AJAX,
    type: "POST",
    data: {
      action: yay_smtp_sendgrid_prefix + "_email_logs",
      nonce: yay_smtp_sendgrid_wp_data.ajaxNonce,
      params: param
    },
    beforeSend: function() {
      YaySmtpSendgridSpinner("yaysmtp-body", true);
    },
    success: function(results) {
      ...
      jQuery(yay_smtp_sendgrid_class_obj + " .yaysmtp-body").html(html);

      /** current page - start */
      jQuery(
        yay_smtp_sendgrid_class_obj + " .yay-smtp-content .pag-page-current"
      ...
      /** previous, next button - end */
      YaySmtpSendgridSpinner("yaysmtp-body", false);
    }
  });
}

function YaySmtpSendgridGetParam(param) {
...
function searchYaySmtpSendgridConditionBasicCurrent() {
  let limit = jQuery(
    yay_smtp_sendgrid_class_obj + ".yay-smtp-wrap.mail-logs .pag-per-page-sel"
  ).val();
  let page = jQuery(
  ...
  let status;
  if (
    jQuery(yay_smtp_sendgrid_class_obj + " #yaysmtp_logs_status_not_send").is(
      ":checked"
    )
  ) {
    if (
      jQuery(yay_smtp_sendgrid_class_obj + " #yaysmtp_logs_status_sent").is(
        ":checked"
      )
    ) {
      status = "all";
...
function setYaySmtpSendgridCookie(cname, cvalue, exdays) {
  var d = new Date();
...
function getYaySmtpSendgridCookie(cname) {
  var name = cname + "=";
...
function loadFirstYaySMTPSendgridLogsList() {
  let page = jQuery(
    yay_smtp_sendgrid_class_obj + ".yay-smtp-wrap.mail-logs .pag-page-current"
  ).val();
  let limit = jQuery(
...
function yaysmtp_sendgrid_wrap_url(string) { 
