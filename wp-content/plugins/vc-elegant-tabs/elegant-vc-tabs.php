...
if ( ! class_exists( 'Elegant_VC_Tabs' ) ) {
	class Elegant_VC_Tabs {
		...
		function render_parent_tab( $atts, $content ) {
			...
			$auto_switch = $switch_interval = $show_hide_tabs = $equal_width_tabs = $switch_on_hover = '';
			...
			foreach ( $shortcode_tabs as $tab ) {
				...
				$tabs_nav .= '<a class="' . $tab_anchor_class . '" style="' . $anchor_style . '" href="#" data-href="#section-' . $tab['tab_id'] . '">';
				$tabs_nav .= $tab_icon;
			...
			if ( $equal_width_tabs ) {
				$el_class = ( '' !== $el_class ) ? trim( $el_class ) . ' equal-width-tabs' : 'equal-width-tabs';
			}

			if ( $switch_on_hover ) {
				$el_class = ( '' !== $el_class ) ? trim( $el_class ) . ' et-switch-on-hover' : 'et-switch-on-hover';
			}
			...
			data-switch-interval="' . esc_attr( $switch_interval ) . '" ' . $show_hide_attr . '>
			<nav>
